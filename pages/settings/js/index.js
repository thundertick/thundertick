...
hotkeyElem = document.getElementById('hotkey');
(function(Mousetrap) {
    /**
    ...
    function _restartRecordTimer() {
        ...
        _recordTimer = setTimeout(_finishRecording, 200);
    }
...
chrome.storage.local.get("hotkey", function(obj){
	var key = "";
	if(!obj.hotkey){
...
chrome.storage.onChanged.addListener(function(changes){
    ...
    hotkeyElem.value = changes.hotkey.newValue;

});


hotkeyElem.addEventListener('focus', function(e){
	e.preventDefault();
	mousetrap.record(function(sequence) {
		chrome.storage.local.set({hotkey:sequence[0]});
		hotkeyElem.blur();
    });
});

hotkeyElem.addEventListener('keydown', function(e){
	e.preventDefault();
});